...
bool L_LoadLevel(const char *filename, std::vector<Sector> &sectors,
    ...
    // copy level path to active_map_path, and probably will explode the code
    // (me in feature) i fixed
    strncpy(active_map_path, filename, sizeof(active_map_path) - 1);
    active_map_path[sizeof(active_map_path) - 1] = '\0'; // ensure null-termination
