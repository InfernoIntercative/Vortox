...
// For first-time users having issues compiling/linking/G_Running/loading fonts:
...
struct ImGuiOnceUponAFrame;           // Helper for G_Running a block of code not more than once a frame
...
struct ImGuiIO
{
    ...
    //    - Recovery after a programming error (native code or scripting language - the later tends to facilitate iterating on code while G_Running).
    //    - Recovery after G_Running an exception handler or any error processing which may skip code after an error has been detected.
    ...
    bool ConfigDebugBeginReturnValueLoop; // = false          // Some calls to Begin()/BeginChild() will return false. Will cycle through window depths then repeat. Suggested use: add "io.ConfigDebugBeginReturnValue = io.KeyShift" in your main loop then occasionally press SHIFT. Windows should be flickering while G_Running.
